"""Tests for :class:`nexus_guard.middleware.NexusGuardMiddleware`.
...
from nexus_guard import NexusFinOpsGuard, SecurityBlockException  # noqa: E402
from nexus_guard.middleware import NexusGuardMiddleware  # noqa: E402
...
def _embedded_guard(**kwargs) -> NexusFinOpsGuard:
    # report=False keeps the test fully offline (no dashboard POST threads).
    return NexusFinOpsGuard(mode="embedded", report=False, **kwargs)


def test_allows_aligned_call_and_runs_handler():
    guard = _embedded_guard(spend_threshold=1000)
    mw = NexusGuardMiddleware(guard, allowed_intents={"buy": "Purchase books under $50"})
...
def test_per_tool_allowed_intent_is_passed_through():
    ...
    mw = NexusGuardMiddleware(guard, allowed_intents={"buy": "only books"})
